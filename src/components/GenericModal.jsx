 ...
 * ✅ CORREÇÃO: Agora aceita a prop 'children' para renderizar conteúdo customizado.
 */
function GenericModal({ isOpen, onClose, title, message, children, isConfirmation = false, onConfirm, theme }) {
    ...
    return (
        <div className="fixed inset-0 bg-gray-600 bg-opacity-50 flex items-center justify-center z-50">
            <div className={`${bgColor} p-6 rounded-lg shadow-xl w-full max-w-sm mx-4 ${textColor}`}>
                ...
                {/* Se o modal recebe 'children', ele renderiza esse conteúdo. */}
                {/* Caso contrário, ele mostra a 'message' e os botões de confirmação. */}
                {children ? (
                    <div>{children}</div>
                ) : (
                    <>
